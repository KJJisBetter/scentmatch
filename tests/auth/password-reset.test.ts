...
describe('Password Reset', () => {
  ...
  describe('Password Update Process', () => {
    it('should validate new password strength', async () => {
      ...
      for (const password of weakPasswords) {
        const result = await updatePassword(password, mockToken, mockRefreshToken)
        expect(result.error).toBeDefined()
    ...
    it('should accept strong passwords', async () => {
      ...
      const result = await updatePassword(strongPassword, mockToken, mockRefreshToken)
  ...
  describe('Reset Security Tests', () => {
    it('should enforce rate limiting on reset requests', async () => {
      ...
      expect(rateLimitedResult.error).toContain('Too many password reset requests')
    })
  ...
  describe('Password Update Security', () => {
    it('should prevent password reuse of weak passwords', async () => {
      ...
      const result = await updatePassword(weakPassword, mockToken, mockRefreshToken)
      expect(result.error).toBeDefined()
      expect(result.error).toContain('Password must contain')
    })
    ...
    it('should handle extremely long passwords', async () => {
      ...
      const result = await updatePassword(longPassword, mockToken, mockRefreshToken)
      expect(result.error).toBeDefined()
    ...
    it('should sanitize password input', async () => {
      ...
      const result = await updatePassword(maliciousPassword, mockToken, mockRefreshToken)
