...
function fixSupabaseTypes(filePath) {
  ...
  const fixedContent = content
    ...
    .replace(
      /(\s+)(await\s+)?supabase\s*\.from\(/g,
      '$1$2(supabase as any).from('
    )
    // Fix cases where it's already wrapped
    .replace(
      /(\s+)(await\s+)?\(supabase as any\) as any\s*\.from\(/g,
      '$1$2(supabase as any).from('
    )
    // Fix createClient calls
    .replace(/(\s+)(await\s+)?createClient\(/g, '$1$2(createClient as any)(')
    // Fix createServerSupabase calls that might need typing
    .replace(
...
// Fix all files
console.log('Fixing Supabase type issues...');
