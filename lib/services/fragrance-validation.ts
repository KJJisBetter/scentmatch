    ...
    try {
      ...
      if (exactMatch && !exactError) {
        ...
        return {
          ...
          brandName: Array.isArray(exactMatch.fragrance_brands) 
          ? exactMatch.fragrance_brands[0]?.name 
          : exactMatch.fragrance_brands?.name,
        };
      }
      ...
      for (const altId of alternativeIds) {
        ...
        if (altMatch && !altError) {
          ...
          return {
            ...
            brandName: Array.isArray(altMatch.fragrance_brands) 
              ? altMatch.fragrance_brands[0]?.name 
              : altMatch.fragrance_brands?.name,
      ...
      if (nameParts.length > 0) {
        ...
        if (nameMatches && nameMatches.length > 0 && !nameError) {
          ...
          return {
            ...
            brandName: Array.isArray(match.fragrance_brands) 
            ? match.fragrance_brands[0]?.name 
            : match.fragrance_brands?.name,
            alternativeIds: [fragranceId, ...alternativeIds],
          };
        }
      }
 ...
 */
export async function validateFragranceIdServer(fragranceId: string): Promise<FragranceValidationResult> {
  try {
    const { createServerSupabase } = await import('@/lib/supabase/server');
    const supabase = await createServerSupabase();
    
    console.log(`🔍 SERVER VALIDATION: Checking fragrance ID "${fragranceId}"`);
    ...
    if (exactMatch && !exactError) {
      console.log(`✅ SERVER EXACT MATCH: Found "${fragranceId}"`);
      return {
        exists: true,
        validId: exactMatch.id,
        fragranceName: exactMatch.name,
    ...
    console.log(`❌ SERVER NOT FOUND: No exact match for "${fragranceId}"`);
    return {
      exists: false,
      validId: null,
    };
