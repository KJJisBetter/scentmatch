...
export class EngagementTriggersService {
  ...
  async analyzeUserBehavior(userId: string): Promise<TriggerAnalysis> {
    ...
    try {
      ...
      const engagementFrequency = this.analyzeEngagementFrequency(
        activityHistory || []
      );
      ...
      const contentPreferences =
        this.analyzeContentPreferences(responseHistory);
  ...
  async optimizeTriggerPerformance(
    ...
    try {
      ...
      const winning_variant = optimization_results.sort(
  ...
  private analyzeEngagementFrequency(
    activityHistory: any[]
    ...
    const daysSinceFirst =
      activityHistory.length > 0
        ? Math.ceil(
            (Date.now() -
              new Date(
                activityHistory[activityHistory.length - 1].created_at
              ).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : 1;
  ...
  private async getTriggerResponseHistory(
    userId: string
  ...
  private calculateOptimalTiming(
    ...
    const avgHour =
      activityHours.length > 0
        ? Math.round(
            activityHours.reduce((sum, hour) => sum + hour, 0) /
              activityHours.length
          )
        : 14; // Default to 2 PM
  ...
  private analyzeContentPreferences(responseHistory: TriggerResponse[]) {
    ...
    const engagedTriggers = responseHistory.filter(
      r => r.response_type === 'engaged'
    );
  ...
  private createContextualTrigger(
    ...
    const triggerTemplates = {
      collection_growth: {
        ...
        message:
          'You have {collection_size} fragrances. Add {needed_for_milestone} more to unlock {reward}!',
        call_to_action: 'Get Recommendations',
      },
      rating_encouragement: {
        title: 'Help us learn your taste! ⭐',
        message:
          'Rate {unrated_count} fragrances to get better recommendations.',
        call_to_action: 'Rate Fragrances',
      },
      social_engagement: {
        title: 'Share your amazing collection! ✨',
        message:
          'Your {collection_size} fragrances are worth sharing with the community.',
    ...
    const template =
      triggerTemplates[templateKey as keyof typeof triggerTemplates] ||
      triggerTemplates.collection_growth;
    ...
    const scheduledFor = this.calculateOptimalDeliveryTime(
      behaviorAnalysis.optimal_timing
    );

    return {
      trigger_id: `contextual_${context.userId || 'anonymous'}_${Date.now()}`,
  ...
  private async generateTriggerPersonalization(
    userId: string
  ): Promise<TriggerPersonalization> {
    const supabase = await this.getSupabase();
    ...
    try {
      ...
      const collectionHighlights =
        collection
          ...
          .map(
            item =>
              `${item.fragrances.name} by ${item.fragrances.fragrance_brands.name}`
          ) || [];
      ...
      const familiesExplored =
        new Set(collection?.map(item => item.fragrances.scent_family)).size ||
        0;
      ...
      return {
        ...
        dynamic_content: {
          ...
          focus_area: this.determineFocusArea(
            collection || [],
            unratedItems.length
          ),
        },
  ...
  private personalizeText(
    ...
    Object.entries(personalization.personalization_data).forEach(
      ([key, value]) => {
  ...
  private selectOptimalTriggerType(
    analysis: TriggerAnalysis
  ): 'email' | 'in_app' | 'push_notification' {
  ...
  private calculateOptimalDeliveryTime(optimalTiming: any): string {
    ...
    const now = new Date();
    const [hour, minute] = optimalTiming.best_time_of_day
      .split(':')
      .map(Number);
  ...
  private determinePriority(
    ...
    if (analysis.user_behavior_profile.engagement_frequency === 'daily')
      return 'medium';
    return 'low';
  }
  ...
  private estimateImpact(context: any, analysis: TriggerAnalysis): number {
    ...
    if (analysis.user_behavior_profile.engagement_frequency === 'daily')
      impact += 20;
    if (analysis.user_behavior_profile.engagement_frequency === 'weekly')
      impact += 10;

    // Adjust based on response history
    const successfulResponses =
      analysis.user_behavior_profile.response_history.filter(
        r => r.response_type === 'engaged'
      ).length;
    const totalResponses =
      analysis.user_behavior_profile.response_history.length;
  ...
  ): 'encouraging' | 'informative' | 'celebratory' | 'urgent' {
  ...
  private determineFocusArea(
    collection: any[],
    unratedCount: number
  ): 'discovery' | 'rating' | 'social' | 'milestone' {
  ...
  private async trackCampaignExecution(
    ...
    try {
      ...
      await supabase.from('collection_analytics_events').insert({
