...
export class MissingProductDetector {
  ...
  private readonly NOTE_PATTERNS: Record<string, string[]> = {
    fresh: ['fresh', 'clean', 'aquatic', 'marine', 'citrus', 'lemon', 'bergamot'],
  ...
  async handleProductNotFound(
    ...
    try {
      ...
      const missingProductId = await this.logMissingProduct(searchQuery, userId, ipAddress, userAgent)
  ...
  async logMissingProduct(
    ...
    try {
      ...
      const { data, error } = await this.supabase.rpc('log_missing_product_request', {
        query: searchQuery,
   ...
   */
  async findAlternatives(searchQuery: string, analysis?: ProductAnalysis): Promise<AlternativeSuggestion[]> {
    ...
    try {
      ...
      if (productAnalysis.notes && alternatives.length < 8) {
        const noteAlternatives = await this.findNoteBasedAlternatives(productAnalysis.notes)
        alternatives.push(...noteAlternatives)
      }

      // Strategy 4: Fuzzy text matching
      if (alternatives.length < 10) {
        const fuzzyAlternatives = await this.findFuzzyTextAlternatives(searchQuery)
  ...
  analyzeSearchQuery(searchQuery: string): ProductAnalysis {
    ...
    for (const [brand, pattern] of Object.entries(this.BRAND_PATTERNS)) {
      if (pattern.test(query)) {
        ...
        if (brandMatch) {
          const afterBrand = query.substring(brandMatch.index! + brandMatch[0].length).trim()
    ...
    if (!extractedBrand) {
      const fragranceIndicators = ['perfume', 'cologne', 'fragrance', 'scent', 'edp', 'edt', 'eau de']
    ...
    // Extract gender
    for (const [genderType, indicators] of Object.entries(this.GENDER_INDICATORS)) {
  ...
  private async findGenderSpecificAlternatives(
    ...
    try {
      ...
      if (excludeBrand) {
        query = query.not('brand_id', 'ilike', `%${excludeBrand.replace(/\s+/g, '-')}%`)
      }
   ...
   */
  private async findNoteBasedAlternatives(notes: string[]): Promise<AlternativeSuggestion[]> {
    try {
      ...
      return fragrances.map(fragrance => {
        const matchingNotes = fragrance.main_accords?.filter(accord => 
        ...
        return {
          ...
          similarity_score: 0.4 + (matchingNotes.length * 0.1) + ((fragrance.popularity_score || 0) * 0.001),
   ...
   */
  private async findFuzzyTextAlternatives(searchQuery: string): Promise<AlternativeSuggestion[]> {
    try {
      ...
      if (error || !fragrances) {
        ...
        const { data: fallbackResults } = await this.supabase
          .from('fragrances')
   ...
   */
  private deduplicateAndRank(alternatives: AlternativeSuggestion[]): AlternativeSuggestion[] {
    // Remove duplicates by fragrance_id
    const unique = alternatives.filter((alt, index, arr) => 
   ...
   */
  private async triggerProductSourcing(searchQuery: string, requestCount: number): Promise<void> {
    try {
      ...
      console.log(`🎯 SOURCING TRIGGERED: "${searchQuery}" reached ${requestCount} requests`)
  ...
  async getMissingProductStats(): Promise<{
    ...
    try {
      ...
      const { count: uniqueProducts } = await this.supabase
        .from('missing_product_summary')
        .select('*', { count: 'exact', head: true })

      // Get top missing products
      const { data: topMissing } = await this.supabase
        .from('missing_product_summary')
      ...
      return {
        ...
        top_missing: topMissing?.map(item => ({
