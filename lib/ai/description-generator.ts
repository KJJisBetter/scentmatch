 ...
 */
export async function generateFragranceDescription(
  ...
  try {
    ...
    return (
      ...
      `A ${fragrance.scent_family} fragrance by ${fragrance.brand}.`
    );
  } catch (error) {
...
export async function getOrCreateDescription(
  ...
  try {
    ...
    try {
      await supabase.from('fragrance_ai_cache').upsert({
...
function createDescriptionPrompt(fragrance: FragranceData): string {
  ...
  const moodText =
    (fragrance.mood_tags?.length ?? 0) > 0
      ? `It's known for being ${fragrance.mood_tags!.join(', ')}.`
      : '';

  const occasionText =
    (fragrance.recommended_occasions?.length ?? 0) > 0
...
export function validateDescription(
  ...
  if (
    !cleaned.toLowerCase().includes(fragrance.name.toLowerCase()) &&
