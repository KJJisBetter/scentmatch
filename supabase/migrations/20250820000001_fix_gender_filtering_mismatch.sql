-- Migration: Fix gender filtering mismatch in get_quiz_recommendations function
-- Issue: RPC function expects 'for men'/'for women' but database has 'men'/'women'
-- Critical launch blocker - SCE-34

-- Drop existing function
DROP FUNCTION IF EXISTS get_quiz_recommendations(TEXT, INTEGER);

-- Recreate with corrected gender filtering logic
...
-- Add logging for debugging (temporary)
CREATE OR REPLACE FUNCTION debug_gender_filtering(target_session_id TEXT)
RETURNS JSON
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  result JSON;
  session_uuid UUID;
  user_gender_value TEXT;
  fragrance_count INTEGER;
BEGIN
  -- Find session
  SELECT id INTO session_uuid FROM user_quiz_sessions WHERE session_token = target_session_id;
  
  -- Get user gender preference
  SELECT answer_value INTO user_gender_value 
  FROM user_quiz_responses 
  WHERE session_id = session_uuid AND question_id = 'gender_preference';
  
  -- Count matching fragrances
  SELECT COUNT(*) INTO fragrance_count
  FROM fragrances f
  WHERE f.sample_available = true
    AND (
      user_gender_value = 'unisex' OR
      (user_gender_value = 'men' AND f.gender IN ('men', 'unisex')) OR
      (user_gender_value = 'women' AND f.gender IN ('women', 'unisex'))
    );
  
  RETURN json_build_object(
    'session_token', target_session_id,
    'session_uuid', session_uuid,
    'user_gender_preference', user_gender_value,
    'matching_fragrances_count', fragrance_count,
    'debug_timestamp', NOW()
  );
END;
$$ LANGUAGE plpgsql;

GRANT EXECUTE ON FUNCTION debug_gender_filtering(TEXT) TO authenticated;