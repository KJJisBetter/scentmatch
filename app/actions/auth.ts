...
const GENERIC_RESET_MESSAGE = 'If an account with that email exists, you will receive a password reset link shortly.'
...
export async function signUp(email: string, password: string) {
  try {
    ...
    if (!emailResult.success) {
      return { error: emailResult.error.issues[0]?.message || 'Invalid email format' }
    }

    if (!passwordResult.success) {
      return { error: passwordResult.error.issues[0]?.message || 'Invalid password format' }
    }
...
export async function signIn(email: string, password: string) {
  try {
    ...
    try {
      ...
      console.error('Failed to ensure user profile during signin:', profileError)
...
export async function resetPassword(email: string) {
  try {
    ...
    if (!rateLimitResult.success) {
      return { error: 'Too many password reset requests. Please try again in 5 minutes.' }
    }
