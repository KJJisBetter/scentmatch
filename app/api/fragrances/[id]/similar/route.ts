...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const threshold = Math.max(
      0,
      Math.min(1, parseFloat(searchParams.get('threshold') || '0.7'))
    );
    const limit = Math.max(
      1,
      Math.min(20, parseInt(searchParams.get('limit') || '6'))
    );
    ...
    const { data: fragranceExists } = await (supabase as any)
      .from('fragrances')
      .select('id')
      .eq('id', id)
      .single();
    ...
    const { data: similarFragrances, error } = await (supabase as any).rpc(
    ...
    if (error) {
      ...
      if (error.code === '42883' || error.code === 'PGRST202') {
        ...
        const { data: allFragrances } = await (supabase as any)
          .from('fragrances')
          .select(
            `
            id,
        ...
        return NextResponse.json(
          ...
          },
          {
            headers: {
              'Cache-Control':
                'public, s-maxage=1800, stale-while-revalidate=3600', // 30 min cache
            },
          }
        );
      }
    ...
    return NextResponse.json(
      ...
      },
      {
        headers: {
          'Cache-Control':
            'public, s-maxage=3600, stale-while-revalidate=86400', // 1 hour cache, 24 hour stale
        },
      }
    );
  } catch (error) {
