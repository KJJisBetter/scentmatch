...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const hasGenderResponse = body.responses.some((r: any) => 
    ...
    if (!hasGenderResponse) {
      console.error(`❌ EARLY VALIDATION: No valid gender preference in request`);
      console.error(`📋 REQUEST RESPONSES: ${JSON.stringify(body.responses.map((r: any) => ({ id: r.question_id, value: r.answer_value })), null, 2)}`);
      return NextResponse.json(
        { 
    ...
    console.log(`💾 STORING QUIZ DATA: Session ${sessionToken} with ${body.responses.length} responses`);
    ...
    try {
      ...
      console.log(`📝 SESSION STORAGE: ${sessionResult.error ? 'FAILED' : 'SUCCESS'} for token ${sessionToken}`);
      ...
      console.log(`🔍 SESSION LOOKUP: ${sessionLookup.error ? 'FAILED' : 'SUCCESS'} - Found UUID: ${sessionLookup.data?.id}`);
      ...
      if (sessionLookup.data?.id) {
        ...
        console.log(`🎯 STORING RESPONSES: ${JSON.stringify(responsesToStore, null, 2)}`);
        
        // MANDATORY: Validate gender preference exists (SCE-81 Deep Fix)
        const genderResponse = responsesToStore.find((r: any) => r.question_id === 'gender_preference');
        if (genderResponse && genderResponse.answer_value) {
          console.log(`👫 GENDER DETECTED: ${genderResponse.answer_value}`);
        } else {
          console.error(`❌ MANDATORY GENDER MISSING: Quiz requests MUST include gender_preference`);
          return NextResponse.json(
            { 
              error: 'Gender preference is required for personalized recommendations',
              error_code: 'MISSING_GENDER_PREFERENCE',
              user_message: 'Your quiz responses are missing a gender preference. Please restart and select For Men, For Women, or Unisex.',
              recovery_action: {
                type: 'restart_quiz', 
                step: 'gender_selection',
        ...
        if (!validGenders.includes(genderResponse.answer_value)) {
          console.error(`❌ INVALID GENDER VALUE: ${genderResponse.answer_value} not in [${validGenders.join(', ')}]`);
          return NextResponse.json(
            { 
        ...
        console.log(`📝 RESPONSES STORAGE: ${responseResult.error ? 'FAILED' : 'SUCCESS'}`);
        if (responseResult.error) {
          console.error(`❌ RESPONSES ERROR:`, responseResult.error);
        } else {
          console.log(`✅ STORED: ${responsesToStore.length} quiz responses for session UUID ${sessionLookup.data.id}`);
        }
        
        // IMPORTANT: Add small delay to ensure database transaction commits before RPC calls
        await new Promise(resolve => setTimeout(resolve, 100));
        console.log(`⏱️ WAIT COMPLETE: Database transaction should be committed`);
      } else {
        console.error(`❌ SESSION UUID NOT FOUND: Cannot store responses without session UUID`);
      }
    } catch (storageError) {
    ...
    const experienceResponse = body.responses.find((r: any) => 
      r.question_id === 'experience_level'
    );
    const isBeginnerUser = experienceResponse?.answer_value === 'beginner' || 
