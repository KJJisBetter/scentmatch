...
import { DirectDatabaseEngine } from '@/lib/quiz/direct-database-engine';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionToken = body.session_token || `quiz-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    // Use direct database engine (working RPC functions)
    const engine = new DirectDatabaseEngine();
    const result = await engine.generateRecommendations(body.responses, sessionToken);
