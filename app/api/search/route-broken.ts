...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!searchResults) {
      ...
      // Apply text search if query provided (search both fragrance names and brand names)
      if (query && query.trim().length > 0) {
        // Try the advanced_fragrance_search function first
        const { data: advancedResults, error: advancedError } = await supabase.rpc('advanced_fragrance_search', {
          query_text: query,
        ...
        if (!advancedError && advancedResults?.length > 0) {
          searchResults = {
            fragrances: advancedResults.map((result: any) => ({
              id: result.fragrance_id,
              name: result.name,
              brand: result.brand,
              brand_id: result.fragrance_id, // Fallback
          ...
          console.log('Advanced search failed, using simple name search');
      ...
      // Only execute fallback query if advanced search didn't work
      if (!searchResults) {
