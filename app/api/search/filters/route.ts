...
export async function GET(request: NextRequest) {
  try {
    ...
    if (scentFamiliesResult.data) {
      scentFamiliesResult.data.forEach(item => {
        if (item.scent_family) {
          scentFamilyCounts[item.scent_family] = (scentFamilyCounts[item.scent_family] || 0) + 1;
        }
      });
    }
    ...
    const brands = (brandsResult.data || [])
      .filter(brand => (brand.item_count || 0) >= 2) // MVP: Only brands with 2+ fragrances
      .slice(0, 10) // Top 10 for MVP
      .map(brand => ({
        value: brand.name,
        label: brand.name,
    ...
    if (occasionsResult.data) {
      occasionsResult.data.forEach(item => {
        if (item.recommended_occasions && Array.isArray(item.recommended_occasions)) {
    ...
    if (seasonsResult.data) {
      seasonsResult.data.forEach(item => {
        if (item.recommended_seasons && Array.isArray(item.recommended_seasons)) {
          item.recommended_seasons.forEach((season: string) => {
            if (season && season.trim()) {
              const normalizedSeason = season.trim();
              seasonCounts[normalizedSeason] = (seasonCounts[normalizedSeason] || 0) + 1;
            }
          });
        }
      });
    ...
    const samplePrices = (sampleStatsResult.data || [])
      .map(item => item.sample_price_usd)
    ...
    if (samplePrices.length > 0) {
      ...
      if (underTen > 0) priceRanges.push({ min: 0, max: 10, label: 'Under $10', count: underTen });
      if (tenToTwenty > 0) priceRanges.push({ min: 10, max: 20, label: '$10-$20', count: tenToTwenty });
      if (twentyToThirty > 0) priceRanges.push({ min: 20, max: 30, label: '$20-$30', count: twentyToThirty });
      if (thirtyPlus > 0) priceRanges.push({ min: 30, max: 1000, label: '$30+', count: thirtyPlus });
    }
