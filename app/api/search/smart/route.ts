...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const fuzzyThreshold = parseFloat(searchParams.get('fuzzy_threshold') || '0.3')
    ...
    if (normalizedQuery.needsNormalization) {
      console.log(`🔄 Query normalized: "${query}" → "${normalizedQuery.canonicalName}"`)
    }
    ...
    try {
      ...
      if (!canonicalError && canonicalResults && canonicalResults.length > 0) {
        ...
        console.log(`✅ Canonical search found ${searchResults.length} results`)
      }
    } catch (error) {
    ...
    if (searchResults.length === 0 && includeFallback) {
      try {
        ...
        if (!functionError && functionResults && functionResults.length > 0) {
          ...
          console.log(`✅ Legacy function found ${searchResults.length} results`)
        }
      } catch (error) {
    ...
    if (searchResults.length === 0 && includeFallback) {
      try {
        ...
        if (!directError && directResults && directResults.length > 0) {
          ...
          console.log(`✅ Direct database found ${searchResults.length} results`)
        }
      } catch (error) {
    ...
    if (searchResults.length === 0) {
      try {
        console.log(`🔍 No results found for "${query}" - triggering missing product intelligence`)
    ...
    const enhancedResults = await enhanceSearchResults(searchResults, query, normalizedQuery)
...
function formatDirectResult(result: any): any {
  const normalizedBrand = normalizeBrandName(result.fragrance_brands?.name || 'Unknown Brand')
...
async function enhanceSearchResults(
  ...
  try {
    ...
    const enhanced = results.map(result => {
      ...
      if (normalizedQuery.needsNormalization) {
        const normalizedTerms = normalizedQuery.canonicalName.toLowerCase().split(' ')
