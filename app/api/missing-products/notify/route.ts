...
export async function POST(request: NextRequest) {
  try {
    ...
    if (email) {
      existingNotificationCheck = await (supabase as any)
    ...
    const { data: notificationRecord, error: insertError } = await (
      supabase as any
    )
      .from('missing_product_notifications')
      .insert({
    ...
    await (supabase as any).from('missing_product_summary').upsert(
      {
        normalized_query: normalizedQuery,
        request_count: 1,
        unique_users: 1,
...
async function getEstimatedWaitingUsers(
