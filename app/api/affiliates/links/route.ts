...
export async function GET(request: NextRequest) {
  try {
    ...
    const links = generateAffiliateLinks(fragranceId, brandName, fragranceName, linkType);
...
function generateAffiliateLinks(
  ...
  const sampleQuery = encodeURIComponent(`${brandName} ${fragranceName} sample`);
  const travelQuery = encodeURIComponent(`${brandName} ${fragranceName} travel size`);
