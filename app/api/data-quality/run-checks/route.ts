...
export async function POST(request: NextRequest) {
  try {
    ...
    if (check_types && Array.isArray(check_types)) {
      const invalidTypes = check_types.filter((type: string) => !validCheckTypes.includes(type))
        ...
        return NextResponse.json(
          {
            ...
            error: {
              ...
              details: { invalid_types: invalidTypes, valid_options: validCheckTypes }
            }
          },
          { status: 400 }
        )
    ...
    if (runAsync) {
      ...
      try {
        ...
        if (resultsError || !results) {
          throw new Error(`Failed to retrieve check results: ${resultsError?.message}`)
        }
