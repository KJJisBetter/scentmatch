...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = Math.max(
      1,
      Math.min(100, parseInt(searchParams.get('limit') || '50'))
    );
    ...
    summary?.forEach((item: any) => {
      ...
      summaryStats.by_severity[item.severity] =
        (summaryStats.by_severity[item.severity] || 0) + item.count;
      summaryStats.by_status[item.status] =
        (summaryStats.by_status[item.status] || 0) + item.count;
    });
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        recommendations: {
          ...
          system_health:
            summaryStats.by_status.open < 10
              ? 'good'
              : summaryStats.by_status.open < 25
                ? 'fair'
                : 'needs_attention',
        },
      },
