...
export async function GET(request: NextRequest) {
  try {
    ...
    if (scoreError || !latestScore) {
      ...
      const { data: newCheckId, error: checkError } = await supabase.rpc('run_data_quality_checks')
      ...
      if (newScoreError || !newScore) {
        throw new Error(`Failed to retrieve quality score: ${newScoreError?.message}`)
      }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        open_issues: {
          ...
          total: Object.values(issueBreakdown).reduce((sum: number, count: number) => sum + count, 0)
        },
        alert_status: {
