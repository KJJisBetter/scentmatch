...
import { Sparkles, Settings, TrendingUp, Compass, Snowflake, RefreshCw } from 'lucide-react';
...
export function RecommendationsSystem({
  ...
  const [recommendations, setRecommendations] = useState<RecommendationSections>({
    perfect_matches: [],
    trending: [],
    adventurous: [],
    seasonal: []
  ...
  }, [userId, collectionStats]);

  // Load AI-powered personalized recommendations
  const loadPersonalizedRecommendationsInternal = async () => {
    const supabase = createClientSupabase();
    ...
    try {
      ...
      setRecommendations({
        perfect_matches: enhanceRecommendations(personalizedRecs || [], 'perfect_matches'),
  ...
  const loadFallbackRecommendations = async () => {
    ...
    if (fallbackRecs) {
      setRecommendations({
        perfect_matches: fallbackRecs.slice(0, 4).map(f => ({ ...f, match_percentage: 75, source: 'fallback' })),
        trending: fallbackRecs.slice(4, 8).map(f => ({ ...f, match_percentage: 70, source: 'fallback' })),
        adventurous: fallbackRecs.slice(8, 10).map(f => ({ ...f, match_percentage: 65, source: 'fallback' })),
        seasonal: fallbackRecs.slice(10, 12).map(f => ({ ...f, match_percentage: 68, source: 'fallback' }))
      });
    }
  };

  // Handle user feedback on recommendations
  const handleFeedback = async (item: any, feedbackType: string, context: string) => {
    try {
      // Optimistic update
      ...
      // Track interaction
      setTrackInteraction({
        type: feedbackType === 'like' ? 'like' : feedbackType === 'dislike' ? 'dislike' : 'sample_request',
      ...
      if (response.ok) {
        ...
        if (result.next_recommendations_affected) {
          // Refresh recommendations if preferences significantly changed
          await loadRecommendations();
        }
        
        // Update user confidence if provided
        if (result.preference_update?.confidence_change) {
          setUserPreferences(prev => ({
            ...prev,
            confidence_level: Math.min(prev.confidence_level + result.preference_update.confidence_change, 1.0)
          }));
        }
      } else {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Collection change detected, refreshing recommendations:', payload);
          loadRecommendations();
        }
      )
      .subscribe();
      ...
      <div className="space-y-12">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  <Badge variant="accent" className="text-xs">
                    {Math.round(userPreferences.confidence_level * 100)}% confident
                  </Badge>
          ...
          <div className="relative">
            <div className="absolute inset-0 bg-background/80 backdrop-blur-sm z-10 flex items-center justify-center">
              <div className="bg-card p-6 rounded-xl shadow-lg border flex items-center space-x-3">
                ...
                <span className="font-medium">Personalizing your recommendations...</span>
              </div>
            </div>
          </div>
        )}
        ...
        <section>
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl font-serif font-bold text-foreground">Perfect Matches</h2>
          ...
          {recommendations.perfect_matches.length > 0 ? (
            <ThemedSections
              ...
              onFeedback={(item, feedback) => handleFeedback(item, feedback, 'perfect_matches')}
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-2">Rate a few fragrances to unlock personalized recommendations</p>
            </div>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl font-serif font-bold text-foreground">Trending in Your Style</h2>
            <Badge variant="accent" className="text-xs">
              Community Picks
            </Badge>
          </div>
          ...
          <ThemedSections
            ...
            onFeedback={(item, feedback) => handleFeedback(item, feedback, 'trending')}
            feedbackState={feedbackState}
        ...
        <section>
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl font-serif font-bold text-foreground">Adventurous Picks</h2>
          ...
          <ThemedSections
            ...
            onFeedback={(item, feedback) => handleFeedback(item, feedback, 'adventurous')}
            feedbackState={feedbackState}
            showExplanations={true}
        ...
        <section>
          ...
          <ThemedSections
            ...
            onFeedback={(item, feedback) => handleFeedback(item, feedback, 'seasonal')}
            feedbackState={feedbackState}
...
function organizeColdStartRecommendations(fragrances: any[]): RecommendationSections {
