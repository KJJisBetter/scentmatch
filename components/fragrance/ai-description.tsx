 ...
 */
export async function AIDescription({
  fragranceId,
  fragrance,
  className = '',
}: AIDescriptionProps) {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* Best Occasions */}
      {fragrance.recommended_occasions &&
        fragrance.recommended_occasions.length > 0 && (
          <div className='mt-4'>
            <h4 className='text-sm font-medium mb-3 text-muted-foreground'>
              Perfect For
            </h4>
...
async function generateSmartDescription(fragrance: {
  ...
  const topNotes =
    fragrance.notes?.slice(0, 3).join(', ') || 'carefully selected notes';
...
function getFragrancePersonality(fragrance: any) {
  ...
  if (family.includes('floral')) {
    return {
      character:
        intensity > 7
          ? 'a bold and captivating floral statement'
          : 'an elegant and refined floral composition',
      appeal:
        'It embodies grace and sophistication with a touch of romantic allure.',
      mood: "an enchanting aura that's both approachable and memorable",
      wearer: 'those',
      values: 'timeless elegance and natural beauty',
      signature:
        'Perfect for making a lasting impression without overwhelming the room.',
    };
  }
  ...
  if (family.includes('woody') || family.includes('amber')) {
    return {
      ...
      appeal:
        'It projects strength and reliability while maintaining an air of mystery.',
  ...
  if (family.includes('fresh') || family.includes('citrus')) {
    return {
      character:
        'a vibrant and energizing fragrance that captures the essence of vitality',
