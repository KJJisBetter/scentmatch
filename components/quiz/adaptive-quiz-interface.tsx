...
export type QuizMode = 'beginner' | 'enthusiast' | 'collector';
...
interface QuizData {
  ...
  adaptive_questions: QuizQuestion[];
}
 ...
 * - Beginner: Simplified language, visual aids, fewer options
 * - Enthusiast: Balanced complexity, moderate terminology
 * - Collector: Advanced terminology, sophisticated options
 */
export function AdaptiveQuizInterface({
  ...
  const questions = quizData.adaptive_questions;
  ...
  const handleMultipleSelectionContinue = () => {
    if (!currentQuestion || !currentQuestion.allowMultiple) return;

    // Validate minimum selections
    if (currentSelections.length < (currentQuestion.minSelections || 1)) {
      return;
    }
