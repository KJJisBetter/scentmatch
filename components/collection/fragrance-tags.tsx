...
export function FragranceTags({
  ...
  const predefinedTags: Record<string, FragranceTag[]> = {
    mood: [
      { id: 'romantic', name: 'Romantic', color: 'pink', category: 'mood', usage_count: 245 },
      { id: 'confident', name: 'Confident', color: 'purple', category: 'mood', usage_count: 189 },
      { id: 'mysterious', name: 'Mysterious', color: 'indigo', category: 'mood', usage_count: 167 },
      { id: 'playful', name: 'Playful', color: 'yellow', category: 'mood', usage_count: 134 },
      { id: 'elegant', name: 'Elegant', color: 'blue', category: 'mood', usage_count: 298 },
      { id: 'fresh', name: 'Fresh', color: 'green', category: 'mood', usage_count: 402 },
    ],
    occasion: [
      { id: 'work', name: 'Work', color: 'gray', category: 'occasion', usage_count: 356 },
      { id: 'date-night', name: 'Date Night', color: 'red', category: 'occasion', usage_count: 267 },
      { id: 'casual', name: 'Casual', color: 'blue', category: 'occasion', usage_count: 445 },
      { id: 'formal', name: 'Formal', color: 'black', category: 'occasion', usage_count: 198 },
      { id: 'party', name: 'Party', color: 'purple', category: 'occasion', usage_count: 123 },
      { id: 'travel', name: 'Travel', color: 'green', category: 'occasion', usage_count: 89 },
    ],
    season: [
      { id: 'spring', name: 'Spring', color: 'green', category: 'season', usage_count: 234 },
      { id: 'summer', name: 'Summer', color: 'yellow', category: 'season', usage_count: 287 },
      { id: 'fall', name: 'Fall', color: 'orange', category: 'season', usage_count: 201 },
      { id: 'winter', name: 'Winter', color: 'blue', category: 'season', usage_count: 178 },
    ],
    time: [
      { id: 'morning', name: 'Morning', color: 'yellow', category: 'time', usage_count: 156 },
      { id: 'afternoon', name: 'Afternoon', color: 'orange', category: 'time', usage_count: 134 },
      { id: 'evening', name: 'Evening', color: 'purple', category: 'time', usage_count: 245 },
      { id: 'night', name: 'Night', color: 'indigo', category: 'time', usage_count: 189 },
    ],
  };
  ...
  return (
    <div className={`space-y-3 ${compact ? '' : 'p-4 border rounded-lg bg-gray-50'}`}>
      {!compact && (
        ...
        <div className="relative">
          ...
          {showSuggestions && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {/* Category Groups */}
              {Object.entries(predefinedTags).map(([categoryName, categoryTags]) => {
                  ...
                  <div key={categoryName} className="border-b border-gray-100 last:border-b-0">
                    ...
                    <div className="p-2">
                      {filteredCategoryTags.slice(0, 5).map((tag) => (
                        ...
                        >
                          <div className="flex items-center space-x-2">
                            <Badge className={`${getTagColorClass(tag.color)} text-xs`}>
                              {tag.name}
                            </Badge>
                          </div>
      ...
      {!compact && (
        <div className="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded px-2 py-1">
          <span className="font-medium">Tips:</span> Press Enter to add • Click tags to remove • 
          {selectedTags.length === 0 ? ' Try popular tags below' : ` ${maxTags - selectedTags.length} slots remaining`}
        </div>
      )}
    </div>
  );
...
export const tagUtils = {
  ...
  extractSmartTags: (fragrance: any): string[] => {
    ...
    if (fragrance.scent_family) {
      smartTags.push(fragrance.scent_family.toLowerCase());
    }
