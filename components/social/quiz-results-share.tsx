...
export function QuizResultsShare({
  ...
  const { user, quiz_results, recommendations, personality_profile, quiz_accuracy } = quizResults;
  ...
  const handleCopyLink = (shareUrl?: string) => {
    const url = shareUrl || `${window.location.origin}/quiz/results/${quiz_results.session_token}`;
      ...
      <Card 
        ...
        style={{
          background: `linear-gradient(135deg, ${
            theme === 'personality' ? '#8B5CF6, #EC4899, #EF4444' :
            theme === 'recommendations' ? '#3B82F6, #8B5CF6, #EC4899' :
        ...
        <CardContent className="p-0 text-white">
          ...
          <div className="p-6 pb-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold">{user.firstName}'s Results</h3>
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full ${confidenceStyle.bg}`}>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {personality_profile.primary_traits.slice(0, 3).map((trait) => (
                      <Badge key={trait} className="bg-white/20 text-white border-white/30">
                        {trait}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  <div className="text-sm opacity-90 mb-2">Favorite Families</div>
                  <div className="flex flex-wrap gap-2">
                    {personality_profile.scent_families.slice(0, 3).map((family) => (
                      <Badge key={family} className="bg-white/20 text-white border-white/30">
                        {family}
                      </Badge>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium capitalize">{personality_profile.intensity_preference}</div>
                  </div>
                  <div>
                    <div className="opacity-90">Complexity</div>
                    <div className="font-medium capitalize">{personality_profile.complexity_style}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 pb-4">
              ...
              <div className="space-y-3">
                {recommendations.slice(0, 3).map((rec, index) => (
                  <div key={rec.fragrance_id} className="flex items-center space-x-3 bg-white/10 rounded-lg p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm opacity-90 truncate">{rec.brand}</div>
                    </div>
                    
                    <div className="text-right">
                      <div className="font-bold">{Math.round((rec.score || 0) * 100)}%</div>
              ...
              <div className="mt-4 text-center">
                <div className="text-sm opacity-90">
                  Discovered {recommendations.length} perfect matches in {Math.round(quiz_results.processing_time / 1000)}s
                </div>
              </div>
            </div>
          )}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white/10 rounded-lg p-4 space-y-3">
                  ...
                  <div className="text-sm">
                    I'm drawn to <strong>{personality_profile.scent_families.slice(0, 2).join(' and ')}</strong> fragrances 
                    with a <strong>{personality_profile.intensity_preference}</strong> intensity preference.
                  </div>
                </div>
                ...
                <div className="bg-white/10 rounded-lg p-4 space-y-3">
                  ...
                  <div className="text-sm">
                    The AI achieved <strong>{Math.round(quiz_accuracy.average_match)}% average accuracy</strong> 
                    with <strong>{quiz_accuracy.confidence_level}</strong> confidence in my matches.
                  </div>
                </div>
                ...
                <div className="bg-white/10 rounded-lg p-4 space-y-3">
                  ...
                  <div className="text-sm">
                    My fragrance personality is <strong>{personality_profile.primary_traits.slice(0, 2).join(' & ')}</strong> 
                    with <strong>{personality_profile.complexity_style}</strong> complexity preferences.
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="px-6 pb-4">
            <div className="grid grid-cols-3 gap-4 text-center text-sm">
              <div>
                <div className="font-bold text-lg">{recommendations.length}</div>
                <div className="opacity-75">Matches Found</div>
              </div>
              <div>
                <div className="font-bold text-lg">{Math.round(quiz_accuracy.average_match)}%</div>
                <div className="opacity-75">Accuracy</div>
              </div>
              <div>
                <div className="font-bold text-lg">{personality_profile.confidence_score}</div>
          ...
          <div className="bg-black/20 p-6 text-center backdrop-blur-sm">
            <div className="space-y-2">
              ...
              <div className="text-sm opacity-90">
                Take the free AI quiz and discover your perfect fragrance matches
              </div>
          ...
          <CardContent className="pt-4">
            <div className="space-y-2">
              ...
              <div className="text-xs text-purple-600 mt-2">
                <strong>Bonus:</strong> First 10 shares unlock premium insights features!
              </div>
            </div>
          </CardContent>
        </Card>
...
export const quizShareUtils = {
  ...
  generateQuizShareData: (
    ...
    return {
      ...
      quiz_results: {
        ...
        recommendation_method: quizResults.recommendation_method || 'unified_ai'
      },
      recommendations: quizResults.recommendations || [],
      personality_profile: {
        primary_traits: personalityInsights?.primary_traits || ['sophisticated', 'adventurous'],
        scent_families: personalityInsights?.dominant_families || ['oriental', 'woody'],
        intensity_preference: personalityInsights?.intensity_preference || 'moderate',
        complexity_style: personalityInsights?.complexity_style || 'varied',
        confidence_score: personalityInsights?.confidence_score || 85
      },
      quiz_accuracy: {
        match_scores: quizResults.recommendations?.map((r: any) => Math.round((r.score || 0) * 100)) || [],
        average_match: quizResults.recommendations?.length > 0 
          ? Math.round(quizResults.recommendations.reduce((sum: number, r: any) => sum + (r.score || 0), 0) / quizResults.recommendations.length * 100)
          : 85,
   ...
   */
  generateSharingText: (data: QuizResultsShareData, platform: string, theme: string) => {
